...
module.exports = function(app, config) {
	function accessCheck(permission) {
		return function(req, res, next) {
			...
			if((!user || !user.can[permission]) && !config.allowAnonymous) {
				res.status(403);
				res.send({
					error: 'Access denied'
				});
	...
	app.get('/:package', accessCheck('read'), function(req, res) {
	...
	app.put('/:package', accessCheck('write'), jsonBodyParser, function(req, res) {
		var params = req.params,
